  ...
  export default {
    ...
    data () {
      return {
        ...
        value: [2019,1,5],//默认选中日期
        begin:[2016,1,1],
        end:[2020,1,1],
        events: {'2019-1-6':'今日备注', '2019-1-8':'一条很长的明日备注'},//时间备注
    ...
    methods:{
      confirm(e){
        ...
        if(this.inputVal){
          wx.request({
            url:`${conf.HOST}/cloud-app/activity.action?method=getActivityList`,
            data:{
              ACTIVITY_STATUS:1,
              pageSize:2,
              pageNo:1,
      ...
      select(val, val2){
        var year,month,date;
        year=val[0];
        month=val[1]<10?"0"+val[1]:val[1];
        date=val[2]<10?"0"+val[2]:val[2];
        var strTime=year+"-"+month+"-"+date;
        //console.log(strTime);
        this.getCurrentDayAct(strTime);
      },
      getCurrentTime(){
        var day=new Date();
      ...
      getVolunDate(){
        ...
        wx.getStorage({
          ...
          success(ret) {
            ...
            wx.request({
              url:`${conf.HOST}/eac-tvmodule/RegistUser.action?method=getVolunDate`,
              data:{
